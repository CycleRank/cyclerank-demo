...
int main(int argc, char* argv[]) {
  ...
  *   );
  * *************************************************************************/
  console->info("CheiRank (alpha={})", alpha);
  ...
  for (unsigned int i=0; i<num_nodes; i++) {
    if (VECTOR(cheirscore)[i] != 0.0) {
      orderedResults.insert(pair<float, int>(VECTOR(cheirscore)[i], i));
    }
  }
