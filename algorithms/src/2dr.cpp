...
struct comp {
    ...
    {
        float prpos_firstElement = firstElement.second.first;
        float cheirpos_firstElement = firstElement.second.second;
        float prpos_secondElement = secondElement.second.first;
        float cheirpos_secondElement = secondElement.second.second;

        float lowpos_firstElement = max(prpos_firstElement, cheirpos_firstElement);
        float lowpos_secondElement = max(prpos_secondElement, cheirpos_secondElement);

        float sumpos_firstElement = prpos_firstElement + cheirpos_firstElement;
        float sumpos_secondElement = prpos_secondElement + cheirpos_secondElement;
        ...
        // finally, if the two elements have the same sum of scores, sort by the one with the lower pr score
        return prpos_firstElement > prpos_secondElement;
    }
};
...
int main(int argc, char* argv[]) {
  ...
  *   );
  * *************************************************************************/
  console->info("Pagerank and CheiRank (alpha={})", alpha);
  ...
  igraph_vector_t prscore, cheirscore;
  ...
  console->debug("Sorting pr and cheirank results");
  map<int, pair<float, float>> scoresCombined;

  for (unsigned int i=0; i<num_nodes; i++) {
    // only insert the intersection of the elements in the two maps
    if (VECTOR(prscore)[i] != 0.0 && VECTOR(cheirscore)[i] != 0.0) {
        scoresCombined.insert(pair<int, pair<float, float>>(i, pair<float, float>(VECTOR(prscore)[i], VECTOR(cheirscore)[i])));
    }
  }
