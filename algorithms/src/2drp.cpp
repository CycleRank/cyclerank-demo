...
// custom sort of a map: https://www.geeksforgeeks.org/sorting-a-map-by-value-in-c-stl/
struct comp {
    template <typename T>
    ...
    {
        float sspprpos_firstElement = firstElement.second.first;
        float cheirpos_firstElement = firstElement.second.second;
        float sspprpos_secondElement = secondElement.second.first;
        float cheirpos_secondElement = secondElement.second.second;

        float lowpos_firstElement = max(sspprpos_firstElement, cheirpos_firstElement);
        float lowpos_secondElement = max(sspprpos_secondElement, cheirpos_secondElement);

        float sumpos_firstElement = sspprpos_firstElement + cheirpos_firstElement;
        float sumpos_secondElement = sspprpos_secondElement + cheirpos_secondElement;

        if (lowpos_firstElement != lowpos_secondElement) {
            // first, sort by the lowest score the two elements have 
            return lowpos_firstElement > lowpos_secondElement;
        }
        else if (sumpos_firstElement != sumpos_firstElement) {
            // then, sort by the lowest sum of scores
            return sumpos_firstElement > sumpos_secondElement;
        }
        // finally, if the two elements have the same sum of scores, sort by the one with the lower ssppr score
        return sspprpos_firstElement > sspprpos_secondElement;
    }
};
...
int main(int argc, char* argv[]) {
  ...
  *    void *options);
  * *************************************************************************/
  console->info("Personalized pagerank and CheiRank (alpha={})", alpha);
  ...
  igraph_vector_t pprscore, cheirscore, reset;
  ...
  {
    console->debug("Calculating the CheiRank on the graph: ");
    ...
    // init result vector
    igraph_vector_init(&cheirscore, 0);
    ...
    */
    ...
    ret=igraph_personalized_pagerank(
         &igrafo,                         // const igraph_t *graph
         IGRAPH_PAGERANK_ALGO_PRPACK,     // igraph_pagerank_algo_t algo
         &cheirscore,                       // igraph_vector_t *vector
         0,                               // igraph_real_t *value
         igraph_vss_all(),                // const igraph_vs_t vids
         directed,                        // igraph_bool_t directed
         pr_alpha,                        // igraph_real_t damping
  ...
  // we now have prscore and cheirscore that hold the data needed to calculate the 2dscore
  // sort the combination of scores by the highest of the two scores. In case of equality, sort by the sum 
  // problem: if they are equal? By definition, the one with the lower pagerank score should triumph over the one with the lower cheirank
  console->debug("Sorting ssppr and cheirank results");
  map<int, pair<float, float>> scoresCombined;

  for (unsigned int i=0; i<num_nodes; i++) {
    // only insert the intersection of the elements in the two maps
    if (VECTOR(pprscore)[i] != 0.0 && VECTOR(cheirscore)[i] != 0.0) {
        scoresCombined.insert(pair<int, pair<float, float>>(i, pair<float, float>(VECTOR(pprscore)[i], VECTOR(cheirscore)[i])));
    }
  }

  set<pair<int, pair<float, float>>, comp> scoresCombinedSorted(scoresCombined.begin(), scoresCombined.end());
  
  scoresCombined.clear();
  ...
  for (auto const& x : scoresCombinedSorted) {
    ...
    if (old2label.find(x.first) == old2label.end()) {
      ...
      fprintf(outfp, "%d\t%d\t%.10e\n", x.first, x.first, 1.0/scoresCounter);
    }
    else {
      //node is present in label system
      fprintf(outfp, "%d\t%s\t%.10e\n", x.first, old2label[x.first].c_str(), 1.0/scoresCounter);
    }
    scoresCounter++;
  }
